...
class TxtDataSet4DualEncoding(data.Dataset):
    ...
    Load captions
    """
    def __init__(self, cap_file, bow2vec, vocab, style, video2frames):
        # Captions
        self.style = style
        self.video2frames = video2frames
        if self.style == 'GT' or self.style == 'GT_new' or self.style == 'GT_withnegative':
            self.captions = {}
            self.cap_ids = []
            self.ref_captions = {}
            with open(cap_file, 'r') as cap_reader:
            ...
            self.vocab = vocab
            self.length = len(self.cap_ids)
        elif self.style == 'distill' or self.style == 'distill_from_best_model' or self.style == 'distill_from_best_model_with_similarity':
            self.captions = {}
            self.cap_ids = []
            with open(cap_file, 'r') as cap_reader:
                for line in cap_reader.readlines():
    ...
    def __getitem__(self, index):
        if self.style == 'GT' or self.style == 'GT_new' or self.style == 'GT_withnegative':
            ...
            captions = self.ref_captions[video_id]
            num_cap = len(captions)
            assert num_cap==20
            cap_bows = torch.zeros(num_cap, self.bow2vec.ndims)
            cap_tensors = []
        ...
        elif self.style == 'GT_withvideo':
            cap_id = self.cap_ids[index]
            video_id = getVideoId(cap_id)

            frames = self.video2frames[video_id]
            captions = self.ref_captions[video_id]
            num_cap = len(captions)
            assert num_cap==20
            cap_bows = torch.zeros(num_cap, self.bow2vec.ndims)
            ...
            return cap_tensor, cap_bow, index, cap_id, cap_tensors, cap_bows, frames
        elif self.style == 'distill' or self.style == 'distill_from_best_model' or self.style == 'distill_from_best_model_with_similarity':
            cap_id = self.cap_ids[index]
...
def get_txt_data_loader(cap_file, vocab, bow2vec, batch_size=100, num_workers=2, style='distill', video2frames=None):
    dset = TxtDataSet4DualEncoding(cap_file, bow2vec, vocab, style, video2frames)

    if style == 'GT' or style == 'GT_new' or style == 'GT_withnegative' or style == 'GT_withvideo':
        data_loader = torch.utils.data.DataLoader(dataset=dset,
                                                batch_size=batch_size,
                                                shuffle=False,
                                                pin_memory=True,
    ...
    elif style == 'distill' or style == 'distill_from_best_model' or style == 'distill_from_best_model_with_similarity':
        data_loader = torch.utils.data.DataLoader(dataset=dset,
                                                batch_size=batch_size,
                                                shuffle=False,
                                                pin_memory=True,
