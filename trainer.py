...
def parse_args():
    ...
    parser.add_argument('--space', type=str, default='latent', help='which concept? hybrid, latent, concept')
    ...
    parser.add_argument('--hidden_size', type=int, default=1024, help='attention hidden size. (default: 1024)')
    ...
    # GT_withnegative means with negative support set
    parser.add_argument('--style', type=str, default='distill_from_best_model', choices=['GT','distill_from_best_model'], help='use GT captions directly or not')
    parser.add_argument('--support_set_number', type=int, default=20, help='use how many GT captions')
    parser.add_argument('--teacher_model', type=str, default='GT', choices=['GT','student'], help='use which model as teacher model')
    parser.add_argument('--student_model', type=str, default='de+map', choices=['de+map','map','text+video'], help='type of student model')
    parser.add_argument('--distill_loss', type=str, default='text', choices=['text','video','text+video'], help='type of student model')
    ...
    parser.add_argument('--text_mapping_layers', type=str, default='0-1536', help='text fully connected layers for common space learning. (default: 0-1536)')
    parser.add_argument('--visual_mapping_layers', type=str, default='0-1536', help='visual fully connected layers  for common space learning. (default: 0-1536)')
    parser.add_argument('--text_resblock_number', type=int, default=0, help='text fully connected resblock number for common space learning')
    # loss
    ...
    parser.add_argument('--distill_with_triplet', action='store_true', help='the second triplet loss')
    parser.add_argument('--distill_type', type=str, default='mse', choices=['mse','kl','mse+kl','cross'], help='distillation loss')
    parser.add_argument('--distill_with_similarity', action='store_true', help='the similarity distillation loss')
    parser.add_argument('--similarity_type', type=str, default=None, help='the type of similarity distillation loss')
    parser.add_argument('--with_detach', action='store_true', help='detach or not')
    parser.add_argument('--finetune_vid', action='store_true', help='finetune video encoding branch or not')
    ...
    parser.add_argument('--alpha', type=float, default=0.2, help='loss balance parameter')
    parser.add_argument('--beta', type=float, default=1, help='loss balance parameter')
    parser.add_argument('--video_alpha', type=float, default=1, help='loss balance parameter')
    ...
    parser.add_argument('--cv_name', type=str, default='support-set', help='')
    #tag
    parser.add_argument('--tag_vocab_size', type=int, default=512, help='what the size of tag vocab will you use')

    parser.add_argument('--test', action='store_true', help='for direct test')
...
def main():
    ...
    if opt.test:
        command = 'python tester.py --collectionStrt %s --testCollection %s --rootpath %s --overwrite %s --logger_name %s' \
                                        % (collectionStrt, collections_pathname['test'], rootpath, str(opt.overwrite), opt.logger_name)
        # if opt.distill_with_triplet:
        #     command += ' --distill_with_triplet'
    ...
    data_loaders = data.get_train_data_loaders(
        caption_files, visual_feats, tag_path, tag_vocab_path, rnn_vocab, bow2vec, opt.support_set_number, opt.batch_size, opt.workers, video2frames)
    ...
    val_text_data_loader = data.get_txt_data_loader(caption_files['val'], rnn_vocab, bow2vec, opt.support_set_number, opt.batch_size, opt.workers, opt.style, video2frames['val'], visual_feats['val'])
    ...
    if opt.resume:
        if os.path.isfile(opt.resume):
            ...
            model.load_state_dict(checkpoint['model'], opt.teacher_model)
    ...
    for epoch in range(opt.num_epochs):
        ...
        logging.info('-'*10)
        # train for one epoch
        train(opt, data_loaders['train'], data_loaders['val'], model, epoch)
...
def train(opt, train_loader, val_loader, model, epoch):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # Update the model
        if opt.style == 'distill_from_best_model':
            if opt.student_model == 'text+video':
                if opt.distill_with_triplet and opt.distill_with_similarity:
                    b_size, loss1, loss2, loss3 = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss1),('Tripletloss', loss2),('Huberloss', loss3)])
                elif opt.distill_with_triplet and not opt.distill_with_similarity:
                    b_size, loss1, loss2 = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss1),('Tripletloss', loss2)])
                elif not opt.distill_with_triplet and opt.distill_with_similarity:
                    b_size, loss1, loss2 = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss1),('Huberloss', loss2)])
                else:
                    b_size, loss1 = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss1)])
            else:
                if opt.distill_with_triplet:
                    b_size, loss1, loss2 = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss1),('Tripletloss', loss2)])
                else:
                    b_size, loss = model.train_emb(i, *train_data)
                    progbar.add(b_size, values=[('MSEloss', loss)])
        elif opt.style=='GT':
            b_size, loss = model.train_emb(i, *train_data)
            progbar.add(b_size, values=[('Tripletloss-1', loss)])
    ...
    if opt.style == 'distill_from_best_model':
        if opt.student_model == 'text+video':
            if opt.distill_with_triplet and opt.distill_with_similarity:
                logging.info('MSEloss: %s, Tripletloss: %s, Huberloss: %s' % (loss1, loss2, loss3))
            elif opt.distill_with_triplet and not opt.distill_with_similarity:
                logging.info('MSEloss: %s, Tripletloss: %s' % (loss1, loss2))
            elif not opt.distill_with_triplet and opt.distill_with_similarity:
                logging.info('MSEloss: %s, Huberloss: %s' % (loss1, loss2))
            else:
                logging.info('MSEloss: %s' % (loss1))
        else:
            if opt.distill_with_triplet:
                logging.info('MSEloss: %s, Tripletloss-2: %s' % (loss1, loss2))
            else:
                logging.info('MSEloss: %s' % (loss))
    elif opt.style=='GT':
        logging.info('Tripletloss: %s' % (loss))
    ...
    if opt.similarity_type == 'adapt':
        ...
        b_size, loss = model.train_mask(i, *val_data)
